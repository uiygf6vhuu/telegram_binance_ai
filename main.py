...
import joblib
from train_ai import train_from_binance, calc_rsi, calc_ema, calc_atr, calc_macd
...
def get_step_size(symbol):
    ...
    try:
        ...
        logger.error(f"Lỗi lấy step size: {str(e)}"); send_telegram(f"⚠️ <b>LỖI STEP SIZE:</b> {symbol} - {str(e)}"); return 0.001
def set_leverage(symbol, lev):
    try:
        ...
        logger.error(f"Lỗi thiết lập đòn bẩy: {str(e)}"); send_telegram(f"⚠️ <b>LỖI ĐÒN BẨY:</b> {symbol} - {str(e)}"); return False
...
def get_balance():
    try:
        ...
        logger.error(f"Lỗi lấy số dư: {str(e)}"); send_telegram(f"⚠️ <b>LỖI SỐ DƯ:</b> {str(e)}"); return 0
def place_order(symbol, side, qty):
    try:
        ...
        logger.error(f"Lỗi đặt lệnh: {str(e)}"); send_telegram(f"⚠️ <b>LỖI ĐẶT LỆNH:</b> {symbol} - {str(e)}"); return None
def cancel_all_orders(symbol):
    try:
        ...
        logger.error(f"Lỗi hủy lệnh: {str(e)}"); send_telegram(f"⚠️ <b>LỖI HỦY LỆNH:</b> {symbol} - {str(e)}"); return False
def get_current_price(symbol):
    try:
        ...
        logger.error(f"Lỗi lấy giá: {str(e)}"); send_telegram(f"⚠️ <b>LỖI GIÁ:</b> {symbol} - {str(e)}"); return 0
def get_positions(symbol=None):
    try:
        ...
        logger.error(f"Lỗi lấy vị thế: {str(e)}"); send_telegram(f"⚠️ <b>LỖI VỊ THẾ:</b> {symbol if symbol else ''} - {str(e)}"); return []
...
class WebSocketManager:
    ...
    def _reconnect(self, symbol, callback):
        logger.info(f"Kết nối lại WebSocket cho {symbol}"); self.remove_symbol(symbol); self._create_connection(symbol, callback)
...
class ProbabilityBot:
    ...
    def load_historical_data(self):
        try:
            df = pd.DataFrame(binance_api_request(f"https://fapi.binance.com/fapi/v1/klines?symbol={self.symbol}&interval=4h&limit=500"), columns=["open_time","open","high","low","close","volume","close_time","quote_asset_volume","number_of_trades","taker_buy_base","taker_buy_quote","ignore"]).astype({"high": float, "low": float, "close": float, "volume": float})
...
class IndicatorBot:
    ...
    def get_signal(self):
        ...
        try:
            df = pd.DataFrame(binance_api_request(f"https://fapi.binance.com/fapi/v1/klines?symbol={self.symbol}&interval=5m&limit=100"), columns=["open_time","open","high","low","close","volume","close_time","quote_asset_volume","number_of_trades","taker_buy_base","taker_buy_quote","ignore"]).astype({"close": float, "high": float, "low": float, "volume": float})
            if df.empty or len(df) < 50:
                return None
            
            # Tính các chỉ báo và đảm bảo lấy giá trị cuối cùng
            ...
            # Lấy giá trị cuối cùng của Series, kiểm tra giá trị None trước khi sử dụng
            rsi_val = rsi.iloc[-1] if rsi is not None and not rsi.empty else None
            ema_fast_val = ema_fast.iloc[-1] if ema_fast is not None and not ema_fast.empty else None
            ema_slow_val = ema_slow.iloc[-1] if ema_slow is not None and not ema_slow.empty else None
            atr_val = atr.iloc[-1] if atr is not None and not atr.empty else None
            macd_val = macd.iloc[-1] if macd is not None and not macd.empty else None
            macd_signal_val = macd_signal.iloc[-1] if macd_signal is not None and not macd_signal.empty else None
            macd_hist_val = macd_hist.iloc[-1] if macd_hist is not None and not macd_hist.empty else None
            volume = df["volume"].iloc[-1]
    
            if None in [rsi_val, ema_fast_val, ema_slow_val, atr_val, macd_val, macd_signal_val, macd_hist_val]:
                return None
            
            features = pd.DataFrame([[rsi_val, ema_fast_val, ema_slow_val, atr_val, macd_val, macd_signal_val, macd_hist_val, volume]],
                                    columns=["RSI","EMA9","EMA21","ATR","MACD","MACD_Signal","MACD_Hist","volume"])
